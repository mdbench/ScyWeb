...
import ScyKernel from '../javascript/ScyKernel.js';

async function runRNSimulation() {
    const dir = "vines_images";
    const path1 = path.join(dir, "rn_vine.ppm");
    const path2 = path.join(dir, "rn_vine.png");
    const testKey = "User";
    ...
    const scy = new ScyKernel(password, path1);

    // Creating the test DBs
    scy._createPPM_DB(path1);
    ...
    if (result === testValue && result2 === testValue) {
        const isValid = scy._validateDB(path1) ? "Valid" : "Invalid";
